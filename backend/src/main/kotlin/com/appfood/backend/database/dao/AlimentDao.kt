...
class AlimentDao {
    ...
    suspend fun findById(id: String): AlimentRow? = dbQuery {
    ...
    suspend fun findByCodeBarres(codeBarres: String): AlimentRow? = dbQuery {
        AlimentsTable.selectAll()
    ...
    suspend fun findBySourceId(source: SourceAliment, sourceId: String): AlimentRow? = dbQuery {
        AlimentsTable.selectAll()
    ...
    suspend fun findBySource(source: SourceAliment, limit: Int = 100, offset: Long = 0): List<AlimentRow> = dbQuery {
        AlimentsTable.selectAll()
    ...
    suspend fun findAll(limit: Int = 100, offset: Long = 0): List<AlimentRow> = dbQuery {
        AlimentsTable.selectAll()
            .limit(limit).offset(offset)
            .map { it.toRow() }
    }
    ...
    suspend fun insert(row: AlimentRow): AlimentRow = dbQuery {
    ...
    suspend fun insertBatch(rows: List<AlimentRow>) = dbQuery {
    ...
    private fun ResultRow.toRow() = AlimentRow(
