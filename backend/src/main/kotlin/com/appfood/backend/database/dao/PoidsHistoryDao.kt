...
class PoidsHistoryDao {
    ...
    suspend fun findByUserId(userId: String, limit: Int = 100): List<PoidsHistoryRow> = dbQuery {
    ...
    suspend fun findLatest(userId: String): PoidsHistoryRow? = dbQuery {
        PoidsHistoryTable.selectAll()
            .where { PoidsHistoryTable.userId eq userId }
            .orderBy(PoidsHistoryTable.date, SortOrder.DESC)
            .limit(1)
