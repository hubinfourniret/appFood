...
class JournalEntryDao {
    ...
    suspend fun insert(row: JournalEntryRow): JournalEntryRow =
        dbQuery {
    ...
    suspend fun update(row: JournalEntryRow): Boolean =
        dbQuery {
     ...
     */
    suspend fun findRecentAlimentIds(
        userId: String,
        limit: Int = 20,
    ): List<String> =
        dbQuery {
    ...
    suspend fun findByUserAll(userId: String): List<JournalEntryRow> =
        dbQuery {
            JournalEntriesTable.selectAll()
