...
class RecetteDao {
    ...
    suspend fun findById(id: String): RecetteRow? = dbQuery {
    ...
    suspend fun findAll(limit: Int = 100, offset: Long = 0): List<RecetteRow> = dbQuery {
        RecettesTable.selectAll()
    ...
    suspend fun update(row: RecetteRow): Boolean = dbQuery {
    ...
    suspend fun findIngredientsByRecetteId(recetteId: String): List<IngredientRow> = dbQuery {
        IngredientsTable.selectAll()
            .where { IngredientsTable.recetteId eq recetteId }
            .map { it.toIngredientRow() }
    }

    suspend fun insertIngredient(row: IngredientRow): IngredientRow = dbQuery {
    ...
    suspend fun deleteIngredientsByRecetteId(recetteId: String): Int = dbQuery {
        IngredientsTable.deleteWhere { IngredientsTable.recetteId eq recetteId }
    }

    private fun ResultRow.toRow() = RecetteRow(
    ...
    private fun ResultRow.toIngredientRow() = IngredientRow(
