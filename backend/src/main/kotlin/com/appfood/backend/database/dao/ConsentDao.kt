...
class ConsentDao {
    ...
    suspend fun findByUserId(userId: String): List<ConsentRow> = dbQuery {
        ConsentsTable.selectAll()
            .where { ConsentsTable.userId eq userId }
            .map { it.toRow() }
    }

    suspend fun findByUserAndType(userId: String, type: ConsentType): ConsentRow? = dbQuery {
        ConsentsTable.selectAll()
    ...
    suspend fun upsert(row: ConsentRow) = dbQuery {
    ...
    private fun ResultRow.toRow() = ConsentRow(
